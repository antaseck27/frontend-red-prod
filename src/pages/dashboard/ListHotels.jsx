...
const ListHotels = () => {
  ...
  const [hotels, setHotels] = useState([]);
  const [submitting, setSubmitting] = useState(false); // ⚡ nouvel état
  ...
  const handleSubmit = async (e) => {
    ...
    if (submitting) return; // ⚡ empêche double clic
    setSubmitting(true);
    ...
    try {
      ...
      fetchHotels(); // rafraîchit la liste
    } catch (err) {
      console.error("NETWORK ERROR 👉", err);
      alert("Erreur réseau : backend injoignable");
    } finally {
      setSubmitting(false); // 🔄 bouton réactivé
    }
  };
          ...
          <div className="relative bg-white rounded-lg w-full max-w-full md:max-w-5xl p-6 shadow-lg z-10 overflow-y-auto max-h-[95vh] mx-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                ...
                disabled={submitting} // ⚡ empêche double clic
              >
